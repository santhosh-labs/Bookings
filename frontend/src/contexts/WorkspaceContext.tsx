...
export const WorkspaceProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    const setCurrentWorkspaceById = useCallback(async (id: number | null) => {
        ...
        if (ws) {
            try {
                ...
                // Pre-emptively set this to avoid auto-select loops
    ...
    useEffect(() => {
        if (user && workspaces.length > 0 && !currentWorkspace && !localStorage.getItem("selected_workspace_id") && !isSwitching.current && !hasAutoSelected.current) {
